...
export default function OnboardingPage() {
  ...
  const { user, signIn, signInWithGoogle, createUserWithEmail } = useAuth();
  ...
  const handleSignInSuccess = async (currentUser: any) => {
    try {
      // Generate handle and create profile
      const handle = generateHandle();
      const displayName = currentUser.displayName || currentUser.email?.split('@')[0] || nickname.trim() || 'User';
            ...
            {!showSignInForm ? (
              <div className="space-y-3">
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      await handleSignInSuccess(currentUser);
                    } catch (error) {
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center space-x-3 bg-blue-600 text-white py-3 px-4 rounded-lg hover:bg-blue-700 transition-colors"
                >
